...
function renderBacktest() {
  ...
  const colors = chartColors();
  equityChart?.destroy();
  equityChart = new Chart($('#equityChart'), { type: 'line', data: { labels: result.curve.map(point => point.date), datasets: [{ label: 'Strategy net', data: result.curve.map(point => point.strategy), borderColor: colors.green, pointRadius: 0, borderWidth: 2.5 }, { label: 'Buy & hold', data: result.curve.map(point => point.hold), borderColor: colors.muted, pointRadius: 0, borderWidth: 1.7 }, { label: 'SPY', data: result.curve.map(point => point.spy), borderColor: colors.gold, pointRadius: 0, borderWidth: 1.7 }] }, options: { ...commonChartOptions(), scales: { x: { ticks: { display: false }, grid: { display: false } }, y: { ticks: { color: '#a7b0bd', callback: value => `${value.toFixed(2)}×` }, grid: { color: '#283649' } } }, plugins: { legend: { labels: { color: '#c5ced7', boxWidth: 10 } } } } });
}
